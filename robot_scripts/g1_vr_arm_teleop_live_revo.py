...
# ---------------------------------------------------------------------
# Revo 2 hand trigger control.
#
# These are safe placeholders until we see the actual Revo 2 command API
# on the robot CPU tomorrow.
#
# Control model:
#   left trigger  -> left Revo hand only
#   right trigger -> right Revo hand only
# ---------------------------------------------------------------------

REVO_TRIGGER_DEADZONE = 0.15
REVO_MAX_GRIP = 0.45  # Conservative first-test closure amount.
...
def set_left_revo_hand(trigger_value):
    """
    TODO tomorrow:
    Replace this print with the actual Revo 2 left hand command.
    trigger_value is normalized: 0.0 open, 1.0 grip.
    """
    trigger_value = clamp01(trigger_value)

    if trigger_value < REVO_TRIGGER_DEADZONE:
        grip = 0.0
    else:
        grip = trigger_value * REVO_MAX_GRIP

    # Placeholder only. Do not remove until real Revo API is connected.
    return grip


def set_right_revo_hand(trigger_value):
    """
    TODO tomorrow:
    Replace this print with the actual Revo 2 right hand command.
    trigger_value is normalized: 0.0 open, 1.0 grip.
    """
    trigger_value = clamp01(trigger_value)
    ...
    try:
        while time.time() - start_time < 30.0:
            ...
            while True:
                ...
                left_trigger = clamp01(msg.get("left_trigger", 0.0))
                right_trigger = clamp01(msg.get("right_trigger", 0.0))

                left_revo_grip = set_left_revo_hand(left_trigger)
                right_revo_grip = set_right_revo_hand(right_trigger)
            ...
            row = {
                ...
                "left_revo_grip": float(left_revo_grip),
                "right_revo_grip": float(right_revo_grip),
            }
            ...
            if now - last_print > 0.5:
                print(
                    ...
                    f"deadman={deadman} "
                    f"L_trig={left_trigger:.2f} "
                    f"R_trig={right_trigger:.2f} "
                    f"L_grip={left_revo_grip:.2f} "
                    f"R_grip={right_revo_grip:.2f}"
                )
                last_print = now
